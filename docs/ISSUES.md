...
### 3.1 工作目录存在大量未提交修改
重构前工作树中已存在大量已修改、已删除和未跟踪文件，包括：
- `web/src/api/http.ts` 的未提交修改导致 `http.spec.ts` 测试失败
- `web/src/api/index.ts` 的未提交修改导致 `index.spec.ts` 测试失败
- `web/src/stores/auth.ts` 的未提交修改导致 `auth.spec.ts` 测试失败
...
| 优先级 | 事项 | 对应章节 | 理由 |
|---|---|---|---|
| P0 | 提交工作区存量变更，恢复三个失败单测 | §3.1 | 工作区不干净，一切后续验证都不可信 |
