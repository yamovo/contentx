...
| 12 | 图片/截图内容摄取（视觉摄取管线，[RFC-003](./RFC-003-image-content-ingestion.md)） | **进入路线图：先 RFC 固定边界**，不占当前承诺里程碑 | 提炼自 roster-lens 实践收编（该项目已功能冻结）；强化 Agent 黄金路径（Agent 可把图片摄取为草稿）与自托管 AI 能力；默认关闭、复用 provider 抽象与 AI 外发治理；触发条件：设计伙伴的图片录入需求或 Agent 摄取场景验证，真实 provider 验证与第 6 项同口径 |
...
| 中 | 图片/截图内容摄取 | [RFC-003](./RFC-003-image-content-ingestion.md) 草案已建；dummy vision provider 离线管线、严格 schema 校验、字段出处账本、草稿落库与 MCP 摄取工具，退出条件见 RFC-003 §5–6 |
