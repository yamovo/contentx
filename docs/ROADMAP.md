...
P3-A“生产就绪”整体进度：**Round 1 ✅**（行程 A 完成）。下一轮为 Round 2（备份与恢复生产闭环）。
...
- [ ] 备份端点返回有效 SQL 文件，包含所有业务表
- [ ] 恢复端点能从备份文件还原，行数与原库一致
- [ ] 定时备份按配置执行，保留策略生效
- [ ] 端到端演练在真实容器中完成，记录 Git SHA + 命令 + 结果
- [ ] 非 superadmin 调用备份/恢复端点返回 403
- [ ] 并发备份请求被正确拒绝
- [ ] 文档（SOP.md）更新备份恢复操作步骤

## Round 3：重建可信压测基线 ⏳

> 对应原“行程 C”。目标：三库在同一条件下重跑，产出可信对照。
...
- [ ] 三库结果来自同一 Git SHA、同一数据集规模、同一响应字段
- [ ] 每次运行保存实际 `COUNT(*)`、Git SHA、应用配置和响应体大小
- [ ] 报告由脚本自动生成，无手工抄写单位
- [ ] 原始 JSON 与 Markdown 表格一致性校验通过
- [ ] MySQL 超时/排队根因有实测归因，不再是“待定位”
- [ ] 现有失效历史数据保留为历史快照，新结果独立呈现
- [ ] PROGRESS/ROADMAP 中 7.2 状态更新
