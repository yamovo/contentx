...
|---|---|
| 最新正式版本 | `v1.3.0` |
| 当前分支 | `main`，基线 `b11117c`，包含尚未提交的 PostgreSQL 恢复加固与文档改动 |
| 当前里程碑 | 发布候选收口 |
| 下一里程碑 | 多租户基础 |
| 发布建议 | 暂不发布；PostgreSQL 复验已通过，先审查提交本轮修复并让最终提交通过 CI |
...
1. 审查并提交本轮 PostgreSQL 恢复加固和文档改动。
2. 让最终提交通过远程 CI、OpenAPI 漂移检查和浏览器 E2E。
3. 确定版本号并同步发布说明与镜像标签。

2026-07-30 的 PostgreSQL 演练已覆盖基线 `b11117c` 加本轮未提交修复；在这些改动形成提交前，它是工作树证据，不是最终发布提交证据。

## 6. 下一步

1. 审查并提交 PostgreSQL 恢复加固与演练文档。
2. 运行最终 CI 并修复任何环境差异。
3. 更新版本号和发布说明。
4. 发布完成后启动多租户数据模型设计。
