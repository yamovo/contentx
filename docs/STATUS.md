...
> **2026-09-24 更新：项目功能冻结。** 作者不再主动开发，`v1.4.0` 为最终稳定版本；未完成工作已转为带验收标准的社区 [Issues](https://github.com/yamovo/contentx/issues)。本文件保留为 v1.4.0 的事实快照，不再随开发更新。
...
| 项目 | 当前状态 |
|---|---|
| 最新正式版本 | `v1.4.0`（最终稳定版本） |
| 当前分支 | `main`（`f5f3659`）；多租户 P0、MCP 主体再验证与 RAG 治理收口已随该批次提交 |
| 当前里程碑 | 项目已功能冻结（2026-09-24）；v1.4.0 为最终稳定版，不再规划新里程碑 |
| 发布建议 | 项目功能冻结，不再创建新版本；v1.4.0 可继续正常使用、构建与下载，未完成工作见社区 Issues |
...
- 攻击矩阵现状：Service/Repository（16+ 条 `TestTenantIsolation_*`）、MCP（token 绑定/主体再验证/资源隔离/RAG 权限）、GraphQL（5 条端到端攻击测试）、刷新链（5 项专项测试）均已本地闭环；剩余为远程 CI 验证与开放真实 tenant B 前的验收
...
v1.4.0 的 PostgreSQL 演练、审查修复和 CI 证据已经归档。项目既已功能冻结，以下阻断项不再由作者关闭，保留作为社区贡献者的参考清单（对应 Issues）：
...
**2026-09-24：项目功能冻结，以下统一待办不再由作者推进。** 未完成项已转为带验收标准的社区 Issues（https://github.com/yamovo/contentx/issues），欢迎贡献者认领；RFC-003 等候选方向同样开放给社区。

~~以下为冻结前（2026-08-29）的统一待办，保留作背景：~~
