...
| 驱动 | 10,000 篇 |
|---|---:|
| PostgreSQL | 非实测（基于配置估算） |
| MySQL | 非实测（基于配置估算） |
| SQLite | 非实测（基于配置估算） |

说明：内置搜索索引在启动时全量加载所有文章到应用内存，因此内存主要由文章数量决定，
与数据库驱动关系较小。本表为基于配置的估算值，非实测；如需精确数据，可使用
`scripts/benchmark/sample-memory.ps1` 在无负载下采样 12 次取 min/mean/max。
...
- [x] 汇总三库对照结论，更新 ROADMAP.md Round 3 状态（Round 3 ✅，2026-07-23）

**已知限制**（不再列为待完成项）：

- **三库空闲内存复测未执行**：§5 内存表为基于配置的估算值，非实测。内存采样需在无负载条件下
  运行 `scripts/benchmark/sample-memory.ps1`，目前未纳入自动化流程。如需精确内存基线，
  可按 SOP §4.1 对照原则手动执行。此项不阻塞 Round 3 退出门槛（退出门槛已全部通过）。
