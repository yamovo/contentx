  ...
  <div class="media-page">
    <div class="page-header">
      ...
      <div class="header-actions">
        <el-button @click="showStats = true"><el-icon><DataAnalysis /></el-icon> 统计</el-button>
        <el-upload
    ...
    <el-card shadow="never" class="filter-card">
      <el-form :inline="true">
        <el-form-item>
          <el-select v-model="filters.type" placeholder="文件类型" clearable @change="fetchMedia">
        ...
        <el-form-item>
          <el-select v-model="filters.folder" placeholder="文件夹" clearable @change="fetchMedia">
            <el-option v-for="f in folders" :key="f" :label="f" :value="f" />
          </el-select>
        </el-form-item>
        <el-form-item>
          <el-input v-model="filters.search" placeholder="搜索文件..." clearable @clear="fetchMedia" />
        </el-form-item>
        <el-form-item>
    ...
    <div v-if="viewMode === 'grid'" class="media-grid" v-loading="loading">
      <div v-for="item in media" :key="item.id" class="media-card" @click="selectMedia(item)"
           :class="{ selected: selectedMedia?.id === item.id }">
        <div class="media-preview">
          <img v-if="item.mime_type.startsWith('image/')" :src="item.url" :alt="item.alt" />
        ...
        <div class="media-info">
          <span class="media-name" :title="item.original_name">{{ item.original_name }}</span>
          <span class="media-size">{{ formatSize(item.file_size) }}</span>
        </div>
      </div>
      <el-empty v-if="!media.length && !loading" description="暂无文件" />
    </div>

    <!-- List View -->
    <el-card v-else shadow="never">
      <el-table :data="media" v-loading="loading" @row-click="selectMedia">
        <el-table-column width="60">
          <template #default="{ row }">
            <el-image v-if="row.mime_type.startsWith('image/')" :src="row.url" style="width: 40px; height: 40px; border-radius: 4px" fit="cover" />
            <el-icon v-else :size="24"><Document /></el-icon>
          </template>
        </el-table-column>
        <el-table-column label="文件名" prop="original_name" min-width="200" />
        <el-table-column label="类型" prop="mime_type" width="120" />
    ...
    <!-- Detail Sidebar -->
    <el-drawer v-model="detailVisible" title="文件详情" size="400px">
      <template v-if="selectedMedia">
        <div class="detail-preview">
          <img v-if="selectedMedia.mime_type.startsWith('image/')" :src="selectedMedia.url" />
        </div>
        <el-form label-position="top" class="detail-form">
          <el-form-item label="文件名"><el-input v-model="editForm.filename" disabled /></el-form-item>
          <el-form-item label="标题"><el-input v-model="editForm.title" /></el-form-item>
          <el-form-item label="替代文本"><el-input v-model="editForm.alt" /></el-form-item>
          <el-form-item label="说明"><el-input v-model="editForm.caption" type="textarea" /></el-form-item>
          <el-form-item label="URL"><el-input :model-value="selectedMedia.url" readonly>
            <template #append><el-button @click="copyUrl(selectedMedia!)">复制</el-button></template>
          </el-input></el-form-item>
          <el-form-item>
            <el-button type="primary" @click="updateMedia">保存修改</el-button>
            <el-button type="danger" @click="deleteMedia(selectedMedia!.id)">删除</el-button>
          </el-form-item>
        </el-form>
      </template>
    </el-drawer>
